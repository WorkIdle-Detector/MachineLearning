...
def create_link(
        incoming, network_builder, scope, nonlinearity=tf.compat.v1.nn.elu,
    ...
    if is_first:
        ...
        if summarize_activations:
            tf.compat.v1.summary.histogram(scope+"/activations", network)
...
def create_inner_block(
        incoming, scope, nonlinearity=tf.compat.v1.nn.elu,
        weights_initializer=tf.compat.v1.truncated_normal_initializer(1e-3),
        bias_initializer=tf.compat.v1.zeros_initializer(), regularizer=None,
    ...
    incoming = slim.conv2d(
        ...
        scope=scope + "/1")
    if summarize_activations:
        tf.compat.v1.summary.histogram(incoming.name + "/activations", incoming)
...
def residual_block(incoming, scope, nonlinearity=tf.compat.v1.nn.elu,
...
def _create_network(incoming, reuse=None, weight_decay=1e-8):
    ...
    feature_norm = tf.compat.v1.sqrt(
        tf.compat.v1.constant(1e-8, tf.compat.v1.float32) +
        tf.compat.v1.reduce_sum(tf.compat.v1.square(features), [1], keepdims=True))
...
def main():
    ...
    with tf.compat.v1.Session(graph=tf.compat.v1.Graph()) as session:
        input_var = tf.compat.v1.placeholder(
            tf.compat.v1.uint8, (None, 128, 64, 3), name="images")
        image_var = tf.compat.v1.map_fn(
            lambda x: _preprocess(x), tf.compat.v1.cast(input_var, tf.compat.v1.float32),
            back_prop=False)

        factory_fn = _network_factory()
        features, _ = factory_fn(image_var, reuse=None)
        features = tf.compat.v1.identity(features, name="features")
